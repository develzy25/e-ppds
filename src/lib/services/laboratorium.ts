...
export async function sendAgentHeartbeat(
  ...
  if (!client) {
    ...
    clientId = `pc-${crypto.randomBytes(8).toString('hex')}`;
    ...
    await db
      ...
      .where(eq(labClients.id, clientId));
  }
...
export async function startBillingSession(
  ...
  await db.insert(labSessions).values({
    ...
    tarifId,
    startTime: now,
    ratePerMinute: tarif.hargaPerMenit,
    status: 'Running',
    pondokId,
    createdAt: now,
  });
...
export async function pauseBillingSession(sessionId: string): Promise<void> {
  ...
  await db
    ...
    .set({
      ...
      totalAmount: diffMinutes * session.ratePerMinute,
    })
    .where(eq(labSessions.id, sessionId));
  ...
  await db
    ...
    .where(eq(labClients.id, session.clientId));
}
...
export async function finishBillingSession(
  ...
  const totalMinutes = (session.durationMinutes || 0) + currentMinutes;
  const totalAmount = totalMinutes * session.ratePerMinute;
...
export async function handlePowerOutage(pondokId: string): Promise<number> {
  ...
  const offlineSessions = await db
    .select({
      ...
      clientId: labSessions.clientId,
    })
    .from(labSessions)
    .innerJoin(labClients, eq(labSessions.clientId, labClients.id))
    .where(
      and(
