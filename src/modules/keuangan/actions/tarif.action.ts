...
export async function getSemuaTarif() {
  try {
    await requirePermission('keuangan.tarif.read');
...
export async function createTarif(formData: FormData) {
  try {
    await requirePermission('keuangan.tarif.create');
    const user = await getCurrentUser();
    ...
    revalidatePath('/keuangan/master/tarif');
...
export async function updateTarif(formData: FormData) {
  try {
    await requirePermission('keuangan.tarif.update');
    const user = await getCurrentUser();
...
export async function deleteTarif(id: string) {
  try {
    await requirePermission('keuangan.tarif.delete');
    const user = await getCurrentUser();
