...
export class CashService {
  ...
  async recordDepositToTreasurer(
    ...
    return await this.uow.execute(async (repos, trx) => {
      ...
      if (!cashBook) throw new BusinessError('BUSINESS_ERROR', 'Buku kas hari ini belum ada');
      if (cashBook.kasAkhir < amount) throw new BusinessError('BUSINESS_ERROR', 'Saldo kas tidak mencukupi untuk disetor');
  ...
  async recordExpense(
    ...
    return await this.uow.execute(async (repos, trx) => {
      ...
      const cashBook = await cashRepo.getCashBookByDate(dateStr, pondokId, periodId);
      if (!cashBook) throw new BusinessError('BUSINESS_ERROR', 'Buku kas hari ini belum ada');
      if (cashBook.kasAkhir < amount) throw new BusinessError('BUSINESS_ERROR', 'Saldo kas tidak mencukupi');
