...
interface KelasTableProps {
  data: KelasEntity[];
  onEdit: (kelas: KelasEntity) => void;
  onDelete: (kelasId: string) => void;
}

export function KelasTable({ data, onEdit, onDelete }: KelasTableProps) {
  const columns: ColumnDef<KelasEntity>[] = [
    {
      accessorKey: 'name',
      header: 'Nama Kelas / Ruang',
      cell: ({ row }) => <span className="font-semibold">{row.getValue('name')}</span>,
    },
    {
      accessorKey: 'sekolah.name',
      header: 'Sekolah / Madrasah',
      cell: ({ row }) => <span>{row.original.sekolah?.name || '-'}</span>,
    },
    {
