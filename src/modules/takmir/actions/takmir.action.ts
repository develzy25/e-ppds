...
import { TakmirService } from '../services/takmir.service';
import { errorResponse, successResponse } from '@/shared/utils/action-error';

const service = new TakmirService(new UnitOfWork());
...
export async function getTakmirs() {
  try {
    await requirePermission('takmir.takmir.read');
    const user = await getCurrentUser();
    const data = await service.getAllTakmirs(user.pondokId, user.permissions);
